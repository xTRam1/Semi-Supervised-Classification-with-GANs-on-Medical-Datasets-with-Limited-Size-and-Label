...
def get_label_unlabel_dataset(csv_path, img_path, ratio):
    ...
    # Getting the index of the unlabeled images in the csv file.
    unlabeled_img_idx_0 = np.setdiff1d(np.asarray(np.where(labels==0)).reshape(np.count_nonzero(labels==0),), labeled_img_idx_0)
    unlabeled_img_idx_1 = np.setdiff1d(np.asarray(np.where(labels==1)).reshape(np.count_nonzero(labels==1),), labeled_img_idx_1)
    ...
    # Concacenating the positive and negative unlabeled samples and returning their dataset.
    unlabeled_imgs = data.loc[np.hstack((unlabeled_img_idx_0, unlabeled_img_idx_1))]
    unlabeled_dataset = UnlabeledCancerDataset(unlabeled_imgs, img_path)
...
class LabeledCancerDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #if img_as_img.shape[1] == 3:
         #   img_as_img = img_as_img.transpose(0, 2, 3, 1)
...
class UnlabeledCancerDataset(Dataset):
    def __init__(self, unlabeled_csv, img_path):
        self.unlabeled_csv = unlabeled_csv
        self.img_path = img_path

        self.image_arr = np.asarray(self.unlabeled_csv.iloc[:, 0])
        self.data_len = len(self.unlabeled_csv)
